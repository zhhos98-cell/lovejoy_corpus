...
The project asks how late nineteenth- and early twentieth-century historical and human sciences reorganized chronology when a single homogeneous universal history became difficult to sustain.
...
Documented public title path:

`10 Apr 1897: Critical summary of the argument of the Milinda-pañha`

→ `Apr 1897 Proceedings: On the meaning of the Buddhist technical terms ...`
...
## 3. Notebook 005 — internal stratification and mobile primitiveness
...
Lovejoy 1906 isolates an impersonal, communicable, quasi-mechanical efficacy across heterogeneous ethnographic materials and raises the abstraction through several predicate levels:

`mechanism/category`

→ `belief family / Primitive Energetics`

→ `fundamental concept of primitive philosophy`

→ `savage thought / broad collective subject`.
...
- `research_notes/lovejoy_pre1899_method_colonial_politics_primary_source_deep_sweep_batch173.md`

Current result:

Brinton explicitly defines ethnological `primitive` as a **stage of culture**, not literal earliest time. Living peoples can therefore be classed as primitive while remaining chronologically contemporary and historically developed.

Operational sequence:

`primitive != literal earliest time`

→ `primitive = cultural-stage position`

→ `contemporaries can occupy unequal historical positions`

→ `like-stage cases become cross-temporally substitutable`

→ `universal psychology supports comparison`
