# MS38 005 — boundary controls: selector reallocation, ritual morphology, and unsynchronized developmental clocks

Consolidated: 2026-08-26
Status: **CANONICAL THEMATIC DOSSIER**
Supersedes active-tree Batches219, 224 and 225. Original versions remain in Git history.

## Core result

Three separate notebook tests now provide complementary controls against reading `MS38_004_001_061_005` as a single civilization ladder.

1. **Suicide / special death:** the same apparent post-mortem punishment is repeatedly reallocated as the operative selector changes.
2. **Human/animal sacrifice:** visible ritual severity or morphology does not itself identify mechanism or developmental rank.
3. **Agriculture/domestication:** an external economic/subsistence sequence cannot silently serve as a universal chronological scaffold for ritual development.

Together they support a bounded analytical formulation:

> **Lovejoy repeatedly refuses to let one visible outcome, ritual form, or assumed cross-domain chronology do more historical/classificatory work than the relation-specific evidence licenses.**

This does not mean that 005 abandons hierarchy or evolutionary language. The notebook explicitly uses `primitive`, `higher stage`, `more developed peoples`, and related rankings. The point is narrower: the coordinates of hierarchy do not always synchronize.

`selector reallocation`, `ritual morphology`, `chronological non-isomorphism`, `multi-axis stratification`, and `master clock` are project terms, not Lovejoy's vocabulary.

---

## I. Suicide / special death — revisable case allocation

Primary locus: corrected 005 transcription, especially PDF pp.3–6, 11, 14–20.

### Attribution background

Marillier, Mauss and Söderblom already supply much of the determinant field: terrestrial status, ritual qualification, manner of death, clan/social consequences, moral differentiation, contact/contamination, and later moral superposition. Söderblom 1901 also provides a serial selector sequence and explicitly follows Marillier with `d'autres divisions`.

Therefore the strong Lovejoy claim is not category invention.

### The suicide sequence

Across adjacent pages, an apparently punitive afterlife attached to suicide/special death can be tested as:

`manner of death`
→ `ritual/manual qualification`
→ `custom / taboo`
→ `social obligation / clan solidarity`
→ `exclusion / blood-contact consequence`
→ `later imported or higher-stage moral interpretation`
→ only if the outcome tracks **desert**, `strict moral punishment`.

The diagnostic operation is:

> **hold the visible outcome fixed; vary the candidate selector; reassign the case until the relation licensed by the evidence is isolated.**

### Strong examples

- Early pages explicitly refuse to identify differentiated post-mortem destiny with moral reward/punishment merely because outcomes differ.
- Greenland-type material associates favorable destiny with drowning/childbirth/special death while Lovejoy marks the relation as non-moral.
- Suicide is considered through clan solidarity: killing oneself can injure the clan through one of its members; exclusion from social/religious rights can follow without requiring a doctrine of moral desert.
- Later moral conceptions or contact can reassign suicide to special punishment, showing historical superposition rather than a timeless selector.

### Evidence limit

Corrected transcription securely establishes serial conceptual reclassification. Literal pen-level revision — crossings-out, arrows, overwritten labels — remains a page-image question logged under MD-006.

---

## II. Human/animal sacrifice — morphology is not a developmental index

Primary locus: corrected 005 transcription, especially PDF pp.67–76.

### p.67 — magnitude can be political/status amplification

Lovejoy compares ordinary funerary killing with extravagant human sacrifice/massacre associated with rulers. The corrected transcription preserves the analytical possibility that the difference is **political/status** rather than a fundamentally different religious motive: a king has greater power/resources.

Thus:

`more victims / more spectacular violence`
`≠ automatically a more primitive religious mechanism`.

### pp.68–69 — human → animal does not prove advancement

The notebook explicitly asks whether apparent differences represent developmental stages or different social settings. It cautions against ranking cultures by a simple scale and refuses to treat animal sacrifice as automatic evidence of advancement over human sacrifice.

A human victim can belong to an older funerary/provisionary logic tied to a particular dead person; changing victim species does not by itself establish mechanism or stage.

### pp.70–71 — ritualized killing does not prove sacrifice

Killing a rain-maker/diviner for failure can be punishment/accountability rather than sacrifice for rain. Cannibalism, medicinal use, spirit-appropriation and ritual killing likewise require separate classification.

### pp.72–76 — same form, different motive; different form, same function

- funerary killing can provide attendants/service for the dead without communion with a god;
- destruction/burning of property can acquire new explanations over time;
- funerary loyalty is not identical to propitiation;
- a human victim can function as a utilitarian messenger to the dead rather than an object of worship;
- one outward rite can therefore carry provisionary, protective/magical, propitiatory, communicative or later reinterpreted motives.

Two non-equivalences follow:

1. `different forms ≠ different developmental stages`;
2. `same form ≠ same motive/mechanism`.

### Four separable coordinates

At minimum, the notebook permits divergence among:

- material morphology — human/animal victim, number of victims, spectacularity;
- social capacity/status — household versus ruler/king;
- operative mechanism — provision, propitiation, magic, communion, punishment, messaging;
- historical/developmental interpretation — older/later, primitive/higher, inherited/reinterpreted.

This is an internal notebook control against flattening ritual severity into a civilization scale.

Evidence grade: medium to medium-high corrected-transcription control. MD-006 optionally requests pp.69–70 (plus p.67/p.76 if easy) for manuscript-image upgrade.

---

## III. Agriculture/domestication — developmental sequences need not share one clock

Primary locus: corrected 005 transcription, especially PDF pp.101–105; strongest p.103.

### The proposed genealogy under test

The surrounding pages connect cultivated plants, fertility rites, human/animal victims, transfer of life/power, domestic-animal sacrifice and later symbolic/personalized interpretations.

At p.103 Lovejoy sketches a branching relation among cultivated plants, domestic animals and sacrifice and identifies a chronological problem. A theory can require an earlier agrarian/fertility rite leading toward domestic-animal sacrifice while simultaneously relying on a general sequence in which domestication supposedly precedes agriculture.

### The `vicious circle`

The corrected transcription preserves Lovejoy's objection as a **vicious circle**. He refuses to save the theory by treating `domestication before agriculture` as universally true and notes that, in Africa, agriculture can be **anterior to domestication**; domestic animals need not supply agricultural labor.

The methodological consequence is:

> **a ritual genealogy cannot borrow a supposedly universal economic-development sequence unless that sequence is independently established for the relevant historical field.**

This is a chronology audit inside a genealogy.

### pp.104–105 — local sequence remains possible

The notebook does not reject developmental sequence as such:

- p.104 says a historical relation between domestic-animal sacrifice and divine-animal/divine-representative sacrifice must be demonstrated rather than assumed;
- p.105 permits a local movement from quasi-mechanical/magical vegetation-soul operation toward later sacramental sacrifice of a personal/divine representative.

The distinction is therefore:

- **locally demonstrated/warranted sequence** versus
- **imported universal sequence used as an explanatory shortcut**.

This prevents a crude `Lovejoy = anti-evolutionist` reading.

Evidence grade: medium-high corrected-transcription control. MD-006 optionally requests p.103 because the small branching diagram may materially clarify the relation among the proposed sequence and the `vicious circle` objection.

---

## IV. Relation to the larger `primitive = stratification` thesis

005 contains both whole-population hierarchy and internal/cross-domain stratification. These boundary cases make the coexistence precise:

- a social rank is not identical to a moral rank;
- a violent or ritual form is not identical to a developmental rank;
- a subsistence chronology is not identical to a religious chronology;
- an inherited form can receive a later interpretation without becoming historically homogeneous.

The strongest safe conclusion is:

> **`primitive` participates in hierarchy, but the hierarchy is not one perfectly synchronized ladder. 005 distributes historical position across selectors, mechanisms, social capacities, ritual forms and domain-specific chronologies.**

This should be read alongside the 004 control where `viññāṇa` can be temporally antecedent to `nāmarūpa` while logically a subdivision of it, and where borrowed components can receive a distinct `arrangement and application`.

---

## V. Saturation rule

Further 005 case-mining is not warranted merely to collect more examples of `same form ≠ same mechanism`. Reopen only if a new passage changes one of:

- relation-type logic;
- chronology across domains;
- category ownership / scale jump;
- source-provenance politics;
- material revision visible on the manuscript page.

Otherwise additional sacrifice/charm/fetish/cannibalism examples are corroborative, not new conceptual batches.
