# `research_notes/` — active research router

Status: ACTIVE SURFACE / HISTORICAL ROUNDS FROZEN 2026-09-05

This directory is intentionally small. It contains only current production controls, terminal syntheses, and the provenance router needed for active work.

## Production authority

1. `JHI_blog_full_draft_v3_7_clean_submission_2026-09-03.md` — canonical prose draft.
2. `JHI_blog_v3_7_notebook_guide_quellenkritik_calibration_2026-09-03.md` — current calibration/control.
3. `JHI_FINAL_EVIDENCE_GATE_2026-08-27.md` — final evidence gate.
4. `JHI_FULL_REPO_MICRO_REUSE_AUDIT_2026-09-05.md` — current micro-reuse audit; consult only when doing the reuse pass.

## Terminal research authority

- `LOVEJOY_004_TERMINAL_SYNTHESIS.md`
- `LOVEJOY_005_TERMINAL_SYNTHESIS.md`
- `LOVEJOY_1897_1898_PUBLICATION_GENESIS_TERMINAL.md`
- `LOVEJOY_FORMATION_1895_1899_TERMINAL.md`
- `LOVEJOY_1902_1906_EXIT_TERMINAL.md`
- `FROZEN_PROVENANCE_REGISTER.md`

If one of these closes a domain, do not re-run the earlier rounds merely because they exist.

## Frozen history

All files that formerly occupied the flat `research_notes/` directory are preserved unchanged under:

`research_notes/_frozen/snapshot_2026-09-05/`

Use `FROZEN_INDEX.md` before entering that snapshot. The default rule is **do not recurse into `_frozen/`**. Retrieve a historical round only when a terminal synthesis or current draft exposes a specific provenance question that cannot be answered at the active surface.

## Reactivation rule

If frozen material becomes genuinely necessary, do not restore an entire old round series. Extract the needed evidence into the relevant terminal synthesis/current control (with provenance), or create one bounded delta note. The active surface should stay small.
