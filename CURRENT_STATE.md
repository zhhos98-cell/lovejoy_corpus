...
- Core objects: `MS38_004_001_061_004` and `MS38_004_001_061_005`, with immediate source/publication environments.
- Ordinary forward endpoint: **1910**.
- Later material enters only for explicit Buddhism / India / Sanskrit / directly relevant Indic continuity, or as clearly marked external control.
- Manual/browser-dependent evidence is excluded from the calculation of automatic research completeness.

## 3. Current top-line synthesis

The strongest formulation is a sequence of separable operations, not one master method:
...
Do not describe early Lovejoy primarily as inventing analytical categories. The constituent controls are extensively visible upstream in contemporary Indology and Marillier's comparative-religion environment. Lovejoy's stronger documentary delta lies in selection, reweighting, repeated local testing, case allocation, cross-domain recombination, and later scale expansion.

## 4. Claim hierarchy

### 004 / 1898 — relation, provenance and configuration

Primary-secure:

- temporal position can differ from logical inclusion (`viññāṇa` / `nāmarūpa`);
- generic resemblance does not settle system identity;
...
The 004 source-uptake map and canonical delta have been synchronized so that Jacobi 1897, Oldenberg 1897 and Oltramare 1909 are no longer falsely listed as unrecovered targets.

Project shorthand: **chronological non-isomorphism**. This is our term, not Lovejoy's.

### 005 — shared discriminanda, particular redivision and dense case reallocation

Direct-primary attribution is now substantially closed by the complete user-returned Marillier `Survivance` text together with direct Söderblom 1901 same-workshop control.
...
The stronger Lovejoy notebook delta is narrower and documentary:
...
### Marillier / EPHE research apparatus — now actor-near controlled

N. W. Thomas's own 1896 description of the EPHE materially upgrades the workshop claim. Thomas describes Marillier's course through the formula **`work by the pupils, directed by the lecturer`** and specifies a concrete procedure: pupils prepared papers on particular questions, read them in class, received Marillier's comments, and the other pupils discussed them. He also describes the formal diploma as a thesis on a **`critical or historical question`** after supervised study.

Official EPHE 1894–96 reports independently show Marillier's `Religions des peuples non civilisés` course working through comparative myth and ritual problems, including classification, missionary/Biblical contamination, and criticism of inherited anthropological theories.

The defensible institutional formulation is therefore:
...
Söderblom's `d’autres divisions` gives an actor-near example of the research product this pedagogy could generate: inherited problem/source apparatus followed by student-specific redivision rather than doctrinal reproduction.

Thomas's same-teacher relation and diploma-product status are secure. MD-007 now asks only whether his 1898 published diploma article itself already states the later form/meaning/origin or survivance-control operator.

### `primitive` — hierarchy without perfect synchronization
...
- Marillier directly shows that later moral content can be superposed on older structures and remain only partially disentangled from them;
- Söderblom supplies an immediate same-workshop control in which several developments can coexist and conceptual transition does not map mechanically onto one universal civilizational degree.

Best current formulation:

> **`primitive` participates in hierarchy, but the hierarchy is not one perfectly synchronized ladder. Historical position is distributed across relations, components, social settings and domains.**

Do not turn this into a claim that Lovejoy rejected evolutionary hierarchy.
...
The full Fletcher article-level hold is closed. Remaining La Flesche questions are proposition-level archival questions only.
...
### Reception control — Oltramare 1909

The returned complete Oltramare pamphlet has a dedicated Western-interpretation discussion with Warren, Oldenberg, Senart and Deussen directly present. No `Lovejoy` hit surfaced in the full-object OCR pass.

Use only the bounded negative formulation:

> **Lovejoy did not surface in the present OCR reception control, while adjacent major interpreters did.**

This suggests his 1898 intervention was not canonical in this 1909 specialist synthesis; it does not prove Oltramare never read him.

### 1909 → 1910 — explicit positive and negative controls

- Kant 1909→1910 remains the only currently controlled ≤1910 published intellectual self-reclassification passing the stronger threshold: Lovejoy explicitly acknowledges his earlier broad `Kant = evolutionist` statement as an error after closer study and splits the predicate by domain.
- The 1910 formal-logic correction is a genuine authorial erratum but only a local writing/proofreading repair.
...
Generic ≤1910 self-correction searching is saturated.

## 5. Political line — deliberately withheld at one point
...
Institutional publication controls are strong:
...
These are project terms, not Lovejoy's historical vocabulary:
...
- predicate fission;
- resolution increase in historical classification;
...
- generic attempts to prove a fixed `Marillier school doctrine` from student similarity.

Reopen automatic conceptual research only if:

1. a genuinely new primary object appears;
2. the research scope changes;
3. a public-facing claim needs a targeted adversarial check.

### Manual

The first user-returned packet was incorporated on 2026-08-26; see `research_notes/manual_primary_returns_2026-08-26.md`.

Resolved by that packet: MD-002, MD-003, MD-004, MD-005, MD-008, plus the Wiener half of MD-010.

The only living manual queue is `research_notes/manual_download_click_backlog.md`. **Five narrow active controls remain:**

1. MD-001 — 1906 `Democracy in the Twentieth Century` direct pages;
2. MD-006 — original 004/005 notebook page images;
3. MD-007 — Thomas 1898 article-internal survivance/form/meaning/origin test only; same-teacher/workshop relation already secure;
4. MD-009 — exact 1898–99 scolarité cote/register only; broad Vth Section scolarité range already secure;
5. MD-010 — JRAS 29.2 (1897), 407–410 page text/signature only; exact object and other reception ecology already secure.
...
MD-010's Cambridge object and DOI are independently confirmed as *JRAS* 29.2 (April 1897), pp.407–410. The JRAS contents/front matter shows no reviewer byline for the notice. The BSB/DDB full 1897 volume is `bsb11799352`, URN `urn:nbn:de:bvb:12-bsb11799352-6`; machine retrieval has not yet exposed the four page images/text. T. W. Rhys Davids later points explicitly to these pages while resisting the stronger deduction that Buddhism as a whole derives from Sāṃkhya, but this does not by itself identify him as the reviewer.

Manual gaps are finite evidence upgrades, not evidence that the non-manual line is unfinished.

## 8. Repository organization rule after cleanup

Keep in the active tree:

- primary/corrected transcriptions and archival indexes;
- CSV/JSON matrices, source registers, manifests and evidence tables;
...
## 9. Log — 2026-08-26 consolidation + manual return incorporation

- Batches221–226 synchronized and non-manual conceptual line converged.
- User manual packet incorporated without creating a synthetic Batch227.
- 1905/1906 *Hatchet* controls establish Lovejoy's changing Bulletin role: Managing Editor in 1905, continuing staff in 1906.
...
- Thomas 1896 direct witness upgrades the Marillier workshop claim from inferred pedagogy to actor-near directed pupil research and collective criticism; MD-007 narrowed accordingly.
- Living manual queue remains five items, but three of them are now only narrow text/page-level completions rather than broad evidentiary gaps.

## 10. Restart rule

If no new manual source or genuinely new primary object has arrived, **do not create another Lovejoy conceptual batch**. If new manual evidence arrives, incorporate it claim-by-claim into the relevant thematic memo/state and shrink the manual queue; create a new numbered conceptual batch only if the new object actually changes the model rather than merely upgrading evidence.